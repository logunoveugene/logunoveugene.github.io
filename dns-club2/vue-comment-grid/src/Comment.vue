<template>
  <div class="comments">
    <div class="wrapper" @mouseenter="showHideBar = true; $emit('close-hide-bar',false);" @mouseleave="showHideBar = false; $emit('close-hide-bar',true);" >
      <div class="avatar">
      <img src="https://i.snag.gy/gKOClI.jpg" alt="">
      </div>
      <div class="commentWrapper" v-if="hideMessage">
        ...
        <div class="hiddenMessage"><i>[ Сообщение скрыто ]</i></div>
      </div>
      <div class="commentWrapper" v-show="!hideMessage">
        ...
        <div class="reply" v-show="!beforeUpdate">
          <div @click="handleBeforeReply()">{{ beforeReply ? 'Передумал' : 'Ответить '}}</div>
          <span v-if="replys.length!=0" class="dot">•</span>
          <div v-if="replys.length!=0" @click="showReply">{{ showReplies ? 'Свернуть ответы' : 'Показать ответы ('+replys.length+')'}}</div>
...
<style scoped>
  ...
  .comments>>>.nameWrapper > .expand {
    ...
    border-radius: 3px;
    color: #ccd4d8;
    box-shadow: inset 0 0 0 1px rgba(204, 212, 216, 1);
